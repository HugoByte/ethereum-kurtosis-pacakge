...
util = import_module("../package_io/util.star")
network_keys_and_public_address = constants.NETWORK_PORT_KEYS_AND_IP_ADDRESS

def start_eth_node(plan, network):
    """
    Function to start an Ethereum node, this function will spawn local, and public testnet node for ethereum.

    Args:
        plan (Plan): The Kurtosis plan.
        network: The network parameter for which network to be start (eg. sepolia, local,)
    ...
    if network in constants.PUBLIC_TESTNET_NETWORKS:
        configuration = util.generate_config(plan, network)
        args_with_right_defaults = input_parser.input_parser(plan, configuration)
    else:
